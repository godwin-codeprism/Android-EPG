...
//        linearLayoutManager.setOrientation(HORIZONTAL);
    ...
    /*package*/ void addViewToAdapter(GridItem child, int index) {
        ((ParentGridAdapter) getAdapter()).addView(child, index);
    }

    /*package*/ void removeViewFromAdapter(int index) {
        ((ParentGridAdapter) getAdapter()).removeViewAt(index);
    }

    /*package*/ View getChildAtFromAdapter(int index) {
        return ((ParentGridAdapter) getAdapter()).getView(index);
    }

    /*package*/ int getChildCountFromAdapter() {
        return ((ParentGridAdapter) getAdapter()).getViewCount();
    }

    /*package*/ void setItemCount(int itemCount) {
        ((ParentGridAdapter) getAdapter()).setItemCount(itemCount);
    }
...
//                    onLayout(false, getLeft(), getTop(), getRight(), getBottom());
    ...
    public void scrollToPosition(final int position, final ScrollOptions options) {
        if (options.viewPosition != null) {
            ...
            final ParentGridAdapter adapter = (ParentGridAdapter) getAdapter();
